    ...
    <el-card class="box-card" :body-style="bodystyle" >
      <router-link :to="{path:'detail',query:{uuid:item.uuid,alias:item.alias}}" v-if="productMachine" >
        ...
        <div v-if="show">
          ...
          <div class="val" v-for="(it,indexs) in process" v-if="it.uuid==item.uuid" :key="indexs" >{{it.status === "WHstatus_HadCompletedPercent" ? it.value + "%" : it.value}}</div>
        </div>
        </router-link>
        ...
        <div v-show="!show" >
          ...
          <div class="val" v-show="!echart" v-if="!WHstatus_ExecState">{{item.value=="" ? "***" : item.value}}</div>
  ...
  methods: {
    ...
    setOptions(item) {
      ...
      if (item.status == "WHstatus_Efficiency") {
        ...
        //昨天的就从今天到零点的数据放在离线里
        if(item.time !== undefined && date <= time){
        //今天还没下线
          flag = false;
        } else {
        //昨天已经下线
      ...
      if (item.value !== null && item.value) {
        for (var i = 0; i < item.value.length; i++) {
          ...
          if(flag) {
            ...
            if (item.value[i].status == "Running") {
              ...
              o.value = item.value[i].time + 24 * 3600 * 1000 - 20 * 60 * 1000 - todayRemain;
              }
            }
          }
          arr.push(o);
        ...
        if (item.status == "WHstatus_Efficiency" && todayRemain > 0) {
