<template>
  ...
  <el-form-item class="machine-select"> 
    <span style="color:#000">{{ $t("message.select") }}</span>
    <el-select v-model="item" value-key="alias" style="width:70%" placeholder="">
      <el-option v-for="(item,index) in machinelist" :key=" 'set' + index" :value="item" :label="item.alias" ></el-option>
    </el-select>
  </el-form-item>
  <el-form-item class="line-num"> 
    ...
    <el-dialog
      ...
      width="30%">
      <select v-model="selected" v-show="add" style="width:200px">
  ...
  export default{
    ...
    methods:{
      ...
      detailMqtt(uuid){
        ...
        var detailMqtt = new NcMqttClient(Cookies.get("username"), Cookies.get("password"), function(_1, _2, _3, _4) {
            ar = [_1, _2, _3, _4];
            if(_1 == "WHNC-PHNX-SIGN-1111" && _2=="WHstatus_Efficiency"){
              console.log(ar)
            }
      ...
      styleInit(){
        this.$nextTick(function(){
          this.containerWidth = this.$refs.detail_container.clientWidth;
          let  floatcolnum = this.containerWidth/this.cardWidth
          let intcolnum = parseInt(floatcolnum);
          intcolnum = intcolnum == 0 ? 1 : intcolnum;
    ...
    mounted(){
      ...
      api.allMachineDropList(obj)
        .then(res => {
          if(res.data.result=="success"){
            _this.machinelist = res.data.values;
            //设置默认选中
            _this.item = _this.machinelist.filter((child)=>{
              return child.uuid === _this.uuid;
            })[0];
          }
        });
    ...
    watch:{
      ...
      allMqttStatus(res){
        console.log(res)
        res.forEach(n => {
          this.machineParameterList.forEach(m => {
            if(n.uuid == this.uuid){
