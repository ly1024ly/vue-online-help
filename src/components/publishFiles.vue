<template>
  <div class="published">
    <el-dialog
        ...
        center>
        <div>
          <p>您可以用以下信息的任意一种向他人分享资料：</p><div>手册号：{{pubbookcode}}</div><div>分享码：FKDSGJJG</div><div>分享链接：{{pubbooklink}}</div><div>分享链接二维码：</div><p class="qrcode" style="text-align:center"><canvas id="canvas" ref="canvas"></canvas></p>
        </div>
    ...
    <el-button type="primary" icon="el-icon-sort-up">上线</el-button>
    <el-button type="primary" icon="el-icon-sort-down">下线</el-button>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="editor(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  export default{
    ...
    methods:{
      ...
      del(row){
        ...
        api.delfile({ID:row.ID}).then(res => {
          if(res.data.result == "success"){
